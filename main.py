...
def build_coin_detail_message(data: dict, sym: str, currency: str) -> str:
    ...
    volume = info.get(f"{currency}_24h_vol")
    ts = info.get("last_updated_at")
    cur_upper = currency.upper()

    return (
        f"<b>{sym}</b>  ({cur_upper})\n\n"
        f"Price:        <code>{format_money(price, currency)}</code>\n"
        f"24h change:   <code>{format_change(change)}</code>\n"
        f"Market cap:   <code>{format_compact(market_cap)} {cur_upper}</code>\n"
        f"24h volume:   <code>{format_compact(volume)} {cur_upper}</code>\n\n"
        f"<i>Updated: {format_updated_time(ts)}</i>"
    )
...
async def cmd_start(message: Message):
    ...
    if existing and existing.get("active"):
        text = (
            ...
            f"Coins: <b>{', '.join(existing.get('coins', DEFAULT_COINS))}</b>\n"
            f"Currency: <b>{existing.get('currency', DEFAULT_CURRENCY).upper()}</b>\n"
            f"Delivery time: <b>{format_delivery_time(existing.get('delivery_hour', DEFAULT_DELIVERY_HOUR))}</b>\n\n"
        ...
        text = (
            ...
            "You are subscribed.\n"
            f"You will receive a daily snapshot of <b>{', '.join(DEFAULT_COINS)}</b> in "
            f"<b>{DEFAULT_CURRENCY.upper()}</b> every day at "
            f"<b>{format_delivery_time(DEFAULT_DELIVERY_HOUR)}</b>.\n\n"
            "Quick commands:\n"
...
async def cmd_settime(message: Message, command: CommandObject):
    ...
    save_subscribers()
    await message.answer(f"Delivery time updated to <b>{format_delivery_time(hour)}</b>.")
...
async def cmd_setcurrency(message: Message, command: CommandObject):
    ...
    if not arg or arg not in ALLOWED_CURRENCIES:
        ...
        await message.answer(f"Usage: /setcurrency CODE\n\nSupported: {supported}")
        return

    user["currency"] = arg
    save_subscribers()
    await message.answer(f"Currency updated to <b>{arg.upper()}</b>.")
...
async def cmd_coins(message: Message):
    ...
    coins = user.get("coins", DEFAULT_COINS)
    await message.answer(
        f"Your tracked coins: <b>{', '.join(coins)}</b>\n\n"
...
async def cmd_supportedcoins(message: Message):
    ...
    await message.answer(f"Supported coins:\n\n<code>{html.escape(symbols)}</code>")
...
async def cmd_addcoin(message: Message, command: CommandObject):
    ...
    if arg not in COIN_MAP:
        await message.answer(
            f"<b>{html.escape(arg)}</b> is not in the supported list. Use /supportedcoins to see all symbols."
        )
        return

    coins = user.setdefault("coins", list(DEFAULT_COINS))
    if arg in coins:
        await message.answer(f"<b>{arg}</b> is already in your list.")
        return
    ...
    coins.append(arg)
    save_subscribers()
    await message.answer(f"Added <b>{arg}</b>. Your list: <b>{', '.join(coins)}</b>")
...
async def cmd_removecoin(message: Message, command: CommandObject):
    ...
    coins = user.setdefault("coins", list(DEFAULT_COINS))
    if arg not in coins:
        await message.answer(f"<b>{html.escape(arg)}</b> is not in your list.")
        return
    ...
    if not coins:
        await message.answer(
            f"Removed <b>{arg}</b>. Your list is now empty — use /addcoin SYM or /resetcoins."
        )
    else:
        await message.answer(f"Removed <b>{arg}</b>. Your list: <b>{', '.join(coins)}</b>")
...
async def cmd_resetcoins(message: Message):
    ...
    save_subscribers()
    await message.answer(f"Coin list reset to defaults: <b>{', '.join(DEFAULT_COINS)}</b>")
...
async def cmd_status(message: Message):
    ...
    if user and user.get("active"):
        ...
        text = (
            ...
            f"Coins: <b>{', '.join(user.get('coins', DEFAULT_COINS))}</b>\n"
            f"Currency: <b>{user.get('currency', DEFAULT_CURRENCY).upper()}</b>\n"
            f"Delivery time: <b>{format_delivery_time(user.get('delivery_hour', DEFAULT_DELIVERY_HOUR))}</b>\n"
            f"Subscribed at: <code>{html.escape(registered_at)}</code>"
        )
    else:
